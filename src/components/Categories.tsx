...
export function Categories({ data }: CategoriesProps) {
  ...
  // 3. Scroll to specific index function
  const scrollToCategory = (index: number) => {
    if (scrollRef.current) {
      const container = scrollRef.current;
      const children = container.children;
    ...
    <div className="absolute bottom-8 left-0 right-0 z-20">
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 relative">
        ...
        <div
          ...
          onScroll={handleScroll}
          className={cn(
            "flex items-center gap-3 py-4 overflow-x-auto scrollbar-hide scroll-smooth transition-all",
            "md:justify-center md:overflow-x-visible md:gap-8",
            isMobile ? "snap-x snap-mandatory px-[25%]" : "" // px-[25%] keeps active item centered
          )}
        >
          {data.map((category, i) => {
            if (!category.url || !category.text) return null;
        ...
        {/* DOTS INDICATORS - Visible on Mobile only */}
