...
export function SearchPage() {
  ...
  // Handle auto popup transition for end-date range selection
  const handleCheckInChange = (val: string) => {
    setCheckIn(val);

    // Auto shift if check-out becomes invalid
...
//   // ── Filters State (Fixed Back-Button Rearranging by hydrating fully from URL) ──
...
//   // Sync state if URL changes (Fixes browser Back Button unexpected layout changes)
//   useEffect(() => {
//     setCheckIn(parseApiDateToInput(searchParams.get("checkIn")));
//     setCheckOut(parseApiDateToInput(searchParams.get("checkOut")));
//     setGuests(searchParams.get("occupants") || "");
//     setChildren(searchParams.get("occupants_small") || "");
//     setPets(searchParams.get("pets") === "true");
//     setBedrooms(searchParams.get("bedrooms") || "");
...
//   const handleVisiblePropertiesChange = useCallback((nextVisibleCabins: Property[]) => {
//     applyVisibleProperties(nextVisibleCabins);
//   }, [applyVisibleProperties]);

//   // 📄 FIX: Scroll to top of listing layout on page change
//   const handlePageChange = useCallback((nextPageIndex: number) => {
//     setActivePageIndex(nextPageIndex);
//     setHoveredCardId(null);
...
//   useEffect(() => {
//     return () => {
//       if (listTransitionTimeoutRef.current) clearTimeout(listTransitionTimeoutRef.current);
//       if (mapSyncDebounceTimeoutRef.current) clearTimeout(mapSyncDebounceTimeoutRef.current);
//     };
//   }, []);

//   // 📅 FIX: Date Validation before Search Sync
//   const handleSearch = () => {
//     if (checkIn && checkOut && new Date(checkIn) > new Date(checkOut)) {
//       setDateError("Checkout date must be after Check-in date.");
//       return;
//     }
//     setDateError(null);
...
//     setFilters((prev) => {
//       const updated = { ...prev, [filterKey]: !prev[filterKey] };
//       setActiveFilters(Object.keys(updated).filter((k) => updated[k as keyof typeof updated]));
//       return updated;
//     });
//   };
...
//   const registerPropertyCardRef = useCallback((id: string | number, node: HTMLAnchorElement | null) => {
//     const key = String(id);
//     if (node) {
//       propertyCardRefs.current[key] = node;
//     } else {
...
//   // 🥊 FIX: Enable seamless routing without blocking clicks completely
//   const handleListingCardClick = useCallback((event: React.MouseEvent<HTMLAnchorElement>, id: string | number) => {
//     if (event.metaKey || event.ctrlKey || event.shiftKey || event.altKey) {
//       return;
//     }

//     // Capture slider button clicks so clicking arrows doesn't navigate to detail view unexpectedly
//     const target = event.target as HTMLElement;
//     if (target.closest('.slider-control-btn') || target.tagName === 'BUTTON') {
//       event.preventDefault();
//       return;
//     }

//     if (typeof window !== "undefined" && window.matchMedia("(min-width: 1024px)").matches) {
//       // Allow standard link navigation, but sync the map state simultaneously!
//       setActiveMarkerId(id);
//       setHoveredCardId(id);
//       requestMapFocus(id);
//     }
...
//   const selectClass = "h-9 rounded-md border border-input bg-[#f3f3f5] px-3 py-1 text-sm text-gray-600 shadow-sm focus:outline-none focus:ring-1 focus:ring-ring";
//   const effectiveHoveredId = hoveredCardId ?? activeMarkerId;
...
//             <AlertCircle className="h-4 w-4" />
//             <AlertTitle>Error</AlertTitle>
//             <AlertDescription>We couldn't load the properties at this moment.</AlertDescription>
//           </Alert>
//         </div>
//       )}
...
//       <div className="px-4 xl:px-6 pb-16">
//         <div className="relative">
//           <div className={`grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-2 gap-4 transition-opacity duration-300 ${isListTransitioning ? "opacity-75" : "opacity-100"}`}>
//             {loading
//               ? Array.from({ length: 8 }).map((_, i) => <PropertyCardSkeletonItem key={i} />)
//               : displayedCabins.map((property, index) => (
//                 <div
//                   key={`${listAnimationCycle}-${property.id}`}
//                   className="animate-in fade-in-0 slide-in-from-bottom-1 duration-500"
...
//               {paginationItems.map((item, index) => {
//                 if (item === "ellipsis") {
//                   return <span key={`ellipsis-${index}`} className="px-1 text-2xl leading-none text-gray-600 tracking-[0.2em]">...</span>;
//                 }
//                 const isActivePage = item - 1 === activePageIndex;
//                 return (
//                   <button
...
//                     type="button"
//                     onClick={() => handlePageChange(item - 1)}
//                     className={`flex h-11 w-11 items-center justify-center rounded-full text-[17px] font-medium transition-all duration-200 ${isActivePage ? "bg-gray-900 text-white shadow" : "text-gray-900 hover:bg-gray-100"}`}
//                   >
//                     {item}
//                   </button>
//                 );
...
//     <div className="flex flex-col bg-gray-50">
//       <style>{`
//         .vaul-scrollable { -webkit-overflow-scrolling: touch; }
//         footer { display: none !important; }
//       `}</style>

//       {/* ── Desktop Bar ── */}
//       <div className="bg-white border-b z-20 flex-shrink-0 max-lg:hidden" style={{ boxShadow: "0 1px 8px rgba(0,0,0,0.06)" }}>
//         <div className="px-4 sm:px-6 py-3">
//           <div className="flex flex-wrap gap-2 items-center">
//             <div className="flex-1 min-w-[140px]">
...
//             <div className="flex-1 min-w-[130px]">
//               <Input type="date" value={checkIn} onChange={(e) => { setCheckIn(e.target.value); setDateError(null); }} className="h-9 text-sm" />
//             </div>

//             <div className="flex-1 min-w-[130px]">
//               <Input type="date" value={checkOut} onChange={(e) => { setCheckOut(e.target.value); setDateError(null); }} className="h-9 text-sm" />
//             </div>

//             <div className="flex-1 min-w-[130px]">
//               <select value={bedrooms} onChange={(e) => setBedrooms(e.target.value)} className={selectClass + " w-full"}>
...
//             <Popover>
//               <PopoverTrigger asChild>
//                 <Button variant="outline" size="sm" className="relative rounded-full h-9 px-3 border-gray-300 gap-1.5">
//                   <Filter className="h-3.5 w-3.5" />
//                   <span className="text-sm">Filters</span>
//                   {activeFilterCount > 0 && (
//                     <span className="absolute -top-1.5 -right-1.5 bg-red-500 text-white text-[10px] rounded-full h-4 w-4 flex items-center justify-center font-bold">
...
//                       <div className="space-y-1">
//                         <Label className="text-xs text-gray-500">Guests</Label>
//                         <Input type="number" value={guests} onChange={(e) => setGuests(e.target.value)} min="0" className="h-9" />
//                       </div>
//                       <div className="space-y-1">
//                         <Label className="text-xs text-gray-500">Children</Label>
//                         <Input type="number" value={children} onChange={(e) => setChildren(e.target.value)} min="0" className="h-9" />
//                       </div>
//                     </div>
//                   </div>
...
//                   <div className="space-y-3">
//                     <h3 className="font-semibold text-gray-900 text-xs uppercase tracking-wider">Preferences</h3>
//                     <div className="space-y-2.5">
//                       <div className="flex items-center gap-2">
//                         <Checkbox id="pets_popover" checked={pets} onCheckedChange={(checked) => setPets(checked === true)} />
//                         <Label htmlFor="pets_popover" className="text-sm cursor-pointer">Pets Allowed</Label>
//                       </div>
//                       {[
//                         { key: "hotTub", label: "Hot Tub" },
//                         { key: "lakefront", label: "Lakefront Location" },
...
//                       ].map(({ key, label }) => (
//                         <div key={key} className="flex items-center gap-2">
//                           <Checkbox id={`popover_${key}`} checked={filters[key as keyof typeof filters]} onCheckedChange={() => handleFilterChange(key as keyof typeof filters)} />
//                           <Label htmlFor={`popover_${key}`} className="text-sm cursor-pointer">{label}</Label>
//                         </div>
//                       ))}
//                     </div>
//                   </div>
...
//                       className="flex-1"
//                       onClick={() => {
//                         setFilters({ mountainView: false, lakefront: false, boatDock: false, evCharger: false, hotTub: false });
//                         setActiveFilters([]);
//                         setGuests(""); setChildren(""); setPets(false); setSortBy("price_daily_low");
//                       }}
//                     >
//                       Clear All
//                     </Button>
//                     <Button size="sm" className="flex-1" onClick={handleSearch}>Apply</Button>
//                   </div>
//                 </div>
//               </PopoverContent>
//             </Popover>

//             <Button className="bg-red-500 hover:bg-red-600 text-white px-5 h-9 rounded-full text-sm font-semibold" onClick={handleSearch}>
//               Search
//             </Button>
//           </div>
//         </div>
...
//           <div className="flex relative">
//             {/* Left Listings Pane */}
//             <div className="max-lg:hidden flex-shrink-0 w-[54%]">{listingsContent}</div>

//             {/* Right Sticky Map */}
//             <div className="max-lg:hidden flex-1 sticky" style={{ top: `${MAP_TOP}px`, height: `calc(100vh - ${MAP_TOP}px)`, padding: "12px 12px 12px 4px", minWidth: 0 }}>
//               <div className="w-full h-full rounded-2xl overflow-hidden shadow-sm">
//                 <SearchMap
//                   properties={allCabins}
//                   hoveredId={effectiveHoveredId}
...
//             {/* Mobile View Background Map */}
//             <div className="lg:hidden fixed inset-x-0 bottom-0 z-0 bg-gray-100" style={{ top: `96px` }}>
//               <div className="absolute top-4 left-4 right-4 z-40 flex items-center justify-between bg-white rounded-full shadow-lg px-4 py-3 border border-gray-100">
//                 <button onClick={() => router.back()} className="p-1 -ml-1 rounded-full hover:bg-gray-100 transition-colors">
//                   <ArrowLeft size={20} className="text-gray-800" />
//                 </button>
...
//             {/* Mobile Persistent Static Drawer Handle */}
//             <div
//               className={`lg:hidden fixed bottom-0 left-0 right-0 z-20 flex flex-col items-center justify-start w-full h-[80px] pt-3 bg-white rounded-t-3xl shadow-[0_-4px_24px_rgba(0,0,0,0.1)] border-t border-gray-200 cursor-pointer transition-opacity duration-300 ${drawerOpen ? 'opacity-0 pointer-events-none' : 'opacity-100'}`}
//               onClick={() => { setSnap(0.65); setDrawerOpen(true); }}
//             >
//               <div className="w-10 h-1.5 rounded-full bg-gray-300 mb-2" />
//               <div className="mt-0.5">
//                 <span className="text-[15px] font-[600] text-gray-900">Over {allCabins.length} homes</span>
//               </div>
//             </div>
...
//                     <div className="flex flex-col items-center justify-start w-full h-[52px] pt-3 pb-2 bg-white rounded-t-3xl cursor-grab active:cursor-grabbing border-b border-gray-100 flex-shrink-0" onClick={() => snap === 1 ? setSnap(0.65) : setSnap(1)}>
//                       <div className="w-10 h-1.5 rounded-full bg-gray-300" />
//                     </div>
//                     <div className="flex-1 overflow-y-auto vaul-scrollable bg-white">{listingsContent}</div>
//                   </DrawerPrimitive.Content>
//                 </DrawerPrimitive.Portal>
//               </DrawerPrimitive.Root>
//             </div>
